...
バックグラウンドタスク完了時、通知がメインエージェントの次のターンに自動注入される。XMLには6つのタグが含まれる:
...
| タグ | 用途 |
|------|------|
| `task-notification` | ラッパー要素 |
| `task-id` | 型プレフィックス付きの一意ID (b/a/t) |
| `task-type` | `local_bash`、`local_agent`、または `in_process_teammate` |
| `status` | `completed`、`error`、または `stopped` |
| `summary` | 出力の先頭500文字（クイック判定用） |
| `output-file` | ディスク上の完全出力へのパス |

## 編集不可キューモード

通知XMLブロックは編集不可としてマーク:
...
モデルが注入された通知テキストを変更しようとすることを防ぐ。通知は読み取り専用の構造化データであり、編集可能な対話フローの一部ではない。

## 出力ファイルシステム

バックグラウンドタスクの出力はディスク `.task_outputs/{task_id}.txt` に保存される:
