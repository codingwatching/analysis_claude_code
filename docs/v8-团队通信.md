...
每个 Teammate 被分配一个不同的 ANSI 颜色用于终端输出，在 cyan、yellow、magenta、green、blue 之间循环。这让并行执行时很容易分辨哪个 Teammate 产生了哪些输出。
...
这防止了多个线程（如 Team Lead 和另一个 Teammate）同时写入同一收件箱时的消息损坏。
...
Teammate 获得 `BASE_TOOLS + 任务 CRUD（含 TaskGet）+ SendMessage`——足以完成工作、读取任务详情、更新共享看板、与同伴通信，但无法生成其他代理或管理团队本身。这确保 Team Lead 作为编排者的角色。
...
在 v8 中，Teammate 的循环很直接：接收提示词，工作直到完成，然后关闭。没有空闲循环或自动认领——这些在 v9 中引入。
